...
new class extends Component {
    ...
    // public $is_active = true; // Removed as per request
    ...
    public function openModal($id = null)
    {
        ...
        $this->resetExcept(['search', 'sortField', 'sortDirection', 'perPage', 'showDeletedRecords', 'selectedEmployees', 'selectAll', 'headers', 'employeeTypes', 'employeeWorkStatuses', 'offices', /* add filter properties */ 'filterEmployeeTypeId', 'filterWorkStatusId', 'filterGender', 'filterStatus', 'filterOfficeId', 'filterAppointmentDateStart', 'filterAppointmentDateEnd', 'filterDobStart', 'filterDobEnd']);
        ...
        if ($id) {
            ...
            $employee = $employeeService->getEmployee($id, $this->showDeletedRecords, ['employeeType', 'employeeWorkStatus']); // Eager load relations
            if ($employee) {
    ...
    public function openViewModal($id, EmployeeService $employeeService)
    {
        ...
        $this->viewEmployee = $employeeService->getEmployee($id, true, ['employeeType', 'employeeWorkStatus', 'offices', 'primaryOffice']); // Load necessary relations
    ...
    private function getEmployees(EmployeeService $employeeService): LengthAwarePaginator
    {
        ...
        $filterParams['with'] = ['employeeType', 'employeeWorkStatus', 'primaryOffice']; // Adjust as needed
            ...
            <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                @forelse($employees as $employee)
                    <tr wire:key="employee-{{ $employee->id }}"
                        class="{{ $employee->deleted_at ? 'bg-red-50 dark:bg-red-900/10' : '' }} hover:bg-gray-50 dark:hover:bg-gray-800/70 transition-colors"> {{-- Removed is_active class --}}
                        <td class="p-4 w-8">
                            <x-checkbox wire:model.live="selectedEmployees" value="{{ (string) $employee->id }}"
                                class="checkbox-sm checkbox-primary" />
                        </td>
                        <!-- Employee Info (Name, ID, Type) -->
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="flex-shrink-0 h-11 w-11 mr-4">
                                    <img class="h-11 w-11 rounded-full object-cover shadow-sm"
                                         src="{{ $employee->photo_path ? asset('storage/' . $employee->photo_path) : 'https://ui-avatars.com/api/?name=' . urlencode($employee->first_name . ' ' . $employee->last_name) . '&color=FFFFFF&background=0D8ABC&bold=true' }}"
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <div class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                                        Office: {{ $employee->primaryOffice->first()?->name ?? 'N/A' }}
                                    </div>
                                    <div class="mt-1.5">
                                        {{-- Dynamic Badge Color for Employee Type - Stronger Colors --}}
                                        @php
                                            $type = $employee->employeeType?->name;
                                            $typeColorClasses = match ($type) {
                                                'Permanent' => 'bg-blue-500 text-white',
                                                'Contract' => 'bg-yellow-500 text-black', // Use black text for yellow bg
                                                'Intern' => 'bg-indigo-500 text-white',
                                                default => 'bg-gray-500 text-white', // Default/Fallback
                                            };
                                        @endphp
                                        <span class="px-2 py-0.5 inline-flex text-xs leading-4 font-semibold rounded-full {{ $typeColorClasses }}">
                                            {{ $type ?? 'N/A' }}
                                        </span>
                                    </div>
                                </div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex flex-col">
                                <div class="text-sm text-gray-800 dark:text-gray-200 flex items-center">
                                    <x-icon name="o-envelope" class="w-4 h-4 mr-1.5 text-gray-400 dark:text-gray-500" />
                                    {{ $employee->email ?? 'No Email' }}
                                </div>
                                <div class="text-sm text-gray-600 dark:text-gray-400 mt-1 flex items-center">
                                    <x-icon name="o-phone" class="w-4 h-4 mr-1.5 text-gray-400 dark:text-gray-500" />
                        ...
                        <td class="px-6 py-4 whitespace-nowrap hidden lg:table-cell">
                            {{-- Dynamic Badge Color for Work Status - Stronger Colors --}}
                             @php
                                ...
                                $statusColorClasses = match ($status) {
                                    ...
                                    'Probation' => 'bg-cyan-500 text-white',
                                    'Earned Leave' => 'bg-purple-500 text-white',
                                    'Suspended' => 'bg-yellow-500 text-black', // Use black text for yellow bg
                                    'Resigned' => 'bg-gray-500 text-white',
                                    'Retired' => 'bg-gray-400 text-white',  
                                    'Contract Expired' => 'bg-gray-300 text-white',
                                    'Internship' => 'bg-indigo-500 text-white',
                                    'Demoted' => 'bg-red-600 text-white',
                                    'Transferred' => 'bg-blue-500 text-white',
                                    'Removal From Service' => 'bg-red-700 text-white',
                                    'Relieved' => 'bg-green-600 text-white',

                                    default => 'bg-gray-500 text-white', // Default/Fallback
                                };
                            @endphp
                            <span class="px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full {{ $statusColorClasses }}">
                                {{ $status ?? 'N/A' }}
                            </span>
                        </td>
                        <!-- Actions -->
                ...
                <x-tab name="bio_photo" label="Bio & Photo" icon="o-user-circle" x-cloak>
                    <div class="grid grid-cols-1 gap-4 p-4">
                        ...
                        <div>
                             ...
                             @if($photo_path)
                                <img src="{{ asset('storage/' . $photo_path) }}" alt="Employee Photo" class="h-20 w-20 rounded-full object-cover mb-2">
                             @else
                                <div class="h-20 w-20 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center mb-2">
            ...
            <div class="p-6 space-y-6 bg-white dark:bg-gray-900 rounded-lg">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start pb-4 border-b dark:border-gray-700">
                    ...
                    <div class="flex flex-wrap gap-2 items-center justify-start sm:justify-end w-full sm:w-auto">
                        {{-- Dynamic Badge Color for Employee Type (View Modal) - Stronger Colors --}}
                        @php
                        ...
                        @endphp
                        <span title="Employee Type" class="inline-flex items-center px-3 py-1 shadow rounded-full text-xs font-semibold {{ $viewTypeColorClasses }}">
                            <x-icon name="o-tag" class="h-3 w-3 mr-1" />
                            {{ $viewType ?? 'N/A' }}
                        </span>
                        {{-- Dynamic Badge Color for Work Status (View Modal) - Stronger Colors --}}
                        @php
                        ...
                        @endphp
                         <span title="Work Status" class="inline-flex items-center px-3 py-1 shadow rounded-full text-xs font-semibold {{ $viewStatusColorClasses }}">
                ...
                <!-- Details Grid -->
                <div class="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-5 pt-4">
