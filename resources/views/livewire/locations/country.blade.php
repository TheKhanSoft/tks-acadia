...
<div wire:id="{{ $this->getId() }}">
    ...
    <x-modal wire:model="showModal" :title="$countryId ? 'Edit Country' : 'Add New Country'" box-class="max-w-3xl" separator
        class="mx-auto rounded-xl shadow-2xl mx-10">
        <x-form wire:submit.prevent="save">
            <div class="grid grid-cols-1 sm:grid-cols-2 px-4 gap-x-3 space-y-3">
                {{-- General Information --}}
                <div class="col-span-2 md:col-span-2 text-lg font-semibold border-b mb-2">
                    <x-icon name="o-information-circle" class="w-5 h-5 mr-2" />
                    General Information
                </div>
                <x-input wire:model="name" label="Country Name" placeholder="Country name" icon="o-globe-alt" inline
                    required class="col-span-1" />
                <x-input wire:model="native" label="Native Name" placeholder="Native name" icon="o-language" inline
                    class="col-span-1" />
                <x-input wire:model="nationality" label="Nationality" placeholder="Nationality (e.g. American)"
                    icon="o-user-group" inline class="col-span-1" />
                <x-input wire:model="capital" label="Capital" placeholder="Capital city" icon="o-building-office"
                    inline class="col-span-1" />
            </div>
            <div class="grid grid-cols-2 sm:grid-cols-5 px-4 space-x-3">
                {{-- Codes --}}
                <div class="col-span-5 text-lg font-semibold border-b mb-2">
                    <x-icon name="o-identification" class="w-5 h-5 mr-2" />
                    Codes
                </div>
                ...
                <x-input wire:model="iso2" label="ISO2 Code" placeholder="ISO2 code (e.g. PK)" icon="o-tag" inline
                    class="col-span-1" />
                <x-input wire:model="numeric_code" label="Numeric Code" placeholder="Numeric code (e.g. 840)"
                    icon="o-hashtag" inline class="col-span-1" type="number" />
                <x-input wire:model="phonecode" label="Phone Code" placeholder="Phone code (e.g. 93)" icon="o-phone"
                    inline class="col-span-1" type="number" />
                <x-input wire:model="tld" label="TLD" placeholder="Top-level domain (e.g., .pk)"
                    icon="o-globe-alt" inline class="col-span-1" />
            </div>
            <div class="grid grid-cols-2 sm:grid-cols-4 px-4 space-x-3">
                {{-- Geography --}}
                <div class="col-span-5 text-lg font-semibold border-b mb-2">
                    <x-icon name="o-map" class="w-5 h-5 mr-2" />
                    Geography
                </div>
                <x-input wire:model="region" label="Region" placeholder="Region (e.g. Asia)" icon="o-map" inline
                    class="col-span-1" />
                <x-input wire:model="subregion" label="Subregion" placeholder="Subregion (e.g. South Asia)"
                    icon="o-map" inline class="col-span-1" />
                <x-input wire:model="latitude" label="Latitude" placeholder="Latitude" icon="o-map-pin" inline
                    class="col-span-1" />
                <x-input wire:model="longitude" label="Longitude" placeholder="Longitude" icon="o-map-pin" inline
                    class="col-span-1" />
            </div>
            <div class="grid grid-cols-2 sm:grid-cols-5 px-4 space-x-3">
                {{-- Merged Currency and Other Section --}}
                <div class="col-span-5 text-lg font-semibold border-b mb-2">
                    <x-icon name="o-currency-dollar" class="w-5 h-5 mr-2" />
                    Currency & Other
                </div>
                <x-input wire:model="currency" label="Currency" placeholder="Currency code (e.g. PKR)"
                    icon="o-currency-dollar" inline class="col-span-1" />
                <x-input wire:model="currency_name" label="Currency Name"
                    placeholder="Currency name (e.g. Pakistani Rupee)" icon="o-currency-dollar" inline
                    class="col-span-1" />
                <x-input wire:model="currency_symbol" label="Currency Symbol" placeholder="Currency symbol (e.g. ₨)"
                    icon="o-currency-dollar" inline class="col-span-1" />
                <x-input wire:model="emoji" label="Emoji" placeholder="Emoji (e.g. 🇵🇰)" icon="o-face-smile"
                    inline class="col-span-1" />
                <x-input wire:model="emojiU" label="Emoji Unicode" placeholder="Emoji unicode (e.g. U+1F1F5 U+1F1F0)"
                    icon="o-face-smile" inline class="col-span-1" />
            </div>
        </x-form>
        ...
        <x-slot:actions>
            ...
            <x-button 
                    type="submit" 
                    label="{{ $countryId ? 'Update Country' : 'Create Country' }}"
                    class="btn-primary" 
                    wire:click="save" 
                    wire:loading.attr="disabled" 
                    wire:target="save"
                    :icon="$countryId ? 'o-pencil-square' : 'o-plus'"
                    spinner
                    spinner-label="{{ $countryId ? 'Updating...' : 'Creating...' }}"
                    tooltip-left="{{ $countryId ? 'Update Country' : 'Add New Country' }}"
                    />
    ...
    <!-- View Country Modal -->
    <x-modal wire:model="showViewModal" title="" separator box-class="max-w-2xl"
        ...
        @if ($viewCountry)
            <div class="bg-white dark:bg-gray-900 rounded-lg overflow-hidden">
                ...
                <div class="relative bg-gradient-to-r from-blue-500 to-indigo-600 h-32 flex items-end">
                    @if ($viewCountry->emoji)
                        <div class="absolute top-4 right-4 text-6xl filter drop-shadow-lg">{{ $viewCountry->emoji }}
                        </div>
                    @endif
                    <div class="p-6 w-full">
                        <div class="flex justify-between items-end">
                            <h2 class="text-3xl font-bold text-white">
                                {{ $viewCountry->name }}
                                @if ($viewCountry->native && $viewCountry->native !== $viewCountry->name)
                                    <span
                                        class="text-sm ml-2 font-normal opacity-90">({{ $viewCountry->native }})</span>
                                @endif
                            </h2>
                            @if ($viewCountry->deleted_at)
                                <span
                                    class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200">
                                    <x-icon name="o-archive-box-x-mark" class="h-3 w-3" />
                                    Deleted
                                </span>
                            @endif
                        </div>
                ...
                <!-- Quick Stats -->
                <div class="bg-gray-50 dark:bg-gray-800 p-4 flex flex-wrap justify-between items-center gap-3 text-sm">
                    <div class="flex items-center">
                        <div
                            class="h-8 w-8 bg-blue-100 dark:bg-blue-900 rounded-full flex items-center justify-center mr-2">
                            <x-icon name="o-building-office" class="h-4 w-4 text-blue-600 dark:text-blue-400" />
                        </div>
                        <div>
                            <div class="text-xs text-gray-500 dark:text-gray-400">Capital</div>
                            <div class="font-medium text-gray-900 dark:text-white">
                                {{ $viewCountry->capital ?: 'N/A' }}</div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center">
                        <div
                            class="h-8 w-8 bg-blue-100 dark:bg-blue-900 rounded-full flex items-center justify-center mr-2">
                            <x-icon name="o-globe-alt" class="h-4 w-4 text-blue-600 dark:text-blue-400" />
                        </div>
                        <div>
                            <div class="text-xs text-gray-500 dark:text-gray-400">Region</div>
                            <div class="font-medium text-gray-900 dark:text-white">{{ $viewCountry->region ?: 'N/A' }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center">
                        <div
                            class="h-8 w-8 bg-blue-100 dark:bg-blue-900 rounded-full flex items-center justify-center mr-2">
                            <x-icon name="o-currency-dollar" class="h-4 w-4 text-blue-600 dark:text-blue-400" />
                        </div>
                        <div>
                            <div class="text-xs text-gray-500 dark:text-gray-400">Currency</div>
                            <div class="font-medium text-gray-900 dark:text-white">
                                @if ($viewCountry->currency_symbol && $viewCountry->currency)
                                    {{ $viewCountry->currency_symbol }} ({{ $viewCountry->currency }})
                                @else
                                    {{ $viewCountry->currency ?: 'N/A' }}
                                @endif
                            </div>
                        </div>
                    </div>
                    ...
                    <div x-data="{ activeTab: 'general' }" class="mb-6">
                        <div class="flex overflow-x-auto border-b border-gray-200 dark:border-gray-700">
                            <button @click="activeTab = 'general'"
                                :class="{ 'text-blue-600 dark:text-blue-400 border-b-2 border-blue-600 dark:border-blue-400': activeTab === 'general' }"
                                class="px-4 py-2 text-sm font-medium text-gray-500 dark:text-gray-400 whitespace-nowrap">
                                General
                            </button>
                            <button @click="activeTab = 'geography'"
                                :class="{ 'text-blue-600 dark:text-blue-400 border-b-2 border-blue-600 dark:border-blue-400': activeTab === 'geography' }"
                                class="px-4 py-2 text-sm font-medium text-gray-500 dark:text-gray-400 whitespace-nowrap">
                                Geography
                            </button>
                            <button @click="activeTab = 'codes'"
                                :class="{ 'text-blue-600 dark:text-blue-400 border-b-2 border-blue-600 dark:border-blue-400': activeTab === 'codes' }"
                                class="px-4 py-2 text-sm font-medium text-gray-500 dark:text-gray-400 whitespace-nowrap">
                                Codes
                            </button>
                            <button @click="activeTab = 'currency'"
                                :class="{ 'text-blue-600 dark:text-blue-400 border-b-2 border-blue-600 dark:border-blue-400': activeTab === 'currency' }"
                                class="px-4 py-2 text-sm font-medium text-gray-500 dark:text-gray-400 whitespace-nowrap">
                                Currency
                            </button>
                        </div>
                        ...
                        <!-- General Tab -->
                        <div x-show="activeTab === 'general'" class="py-3 space-y-3">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                ...
                                <div
                                    ...
                                    <div class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">
                                        Country Name</div>
                                    <div class="text-gray-900 dark:text-white font-medium">{{ $viewCountry->name }}
                                    </div>
                                </div>
                                ...
                                <div
                                    ...
                                    <div class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">
                                        Nationality</div>
                                    <div class="text-gray-900 dark:text-white font-medium">
                                        {{ $viewCountry->nationality ?: 'N/A' }}</div>
                                </div>
                                ...
                                <div
                                    ...
                                    <div class="text-gray-900 dark:text-white font-medium flex items-center">
                                        <span class="text-3xl mr-2">{{ $viewCountry->emoji ?: 'N/A' }}</span>
                                        @if ($viewCountry->emojiU)
                                            <span class="text-xs text-gray-500">({{ $viewCountry->emojiU }})</span>
                                        @endif
                                    </div>
                                </div>
                            </div>
                        ...
                        <!-- Geography Tab -->
                        <div x-show="activeTab === 'geography'" class="py-3 space-y-3">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <!-- Capital -->
                                <div
                                    class="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-100 dark:border-gray-700 shadow-sm">
                                    <div class="flex items-center mb-2">
                                        <x-icon name="o-building-office" class="h-4 w-4 text-blue-500 mr-2" />
                                ...
                                <div
                                    class="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-100 dark:border-gray-700 shadow-sm">
                                    <div class="flex items-center mb-2">
                                        <x-icon name="o-globe-asia-australia" class="h-4 w-4 text-blue-500 mr-2" />
                                        <div class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                                            Region</div>
                                    </div>
                                    <div class="text-gray-900 dark:text-white font-medium">
                                        {{ $viewCountry->region ?: 'N/A' }}</div>
                                </div>
                                ...
                                <div
                                    class="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-100 dark:border-gray-700 shadow-sm">
                                    <div class="flex items-center mb-2">
                                        <x-icon name="o-map" class="h-4 w-4 text-blue-500 mr-2" />
                                        <div class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                                            Subregion</div>
                                    </div>
                                    <div class="text-gray-900 dark:text-white font-medium">
                                        {{ $viewCountry->subregion ?: 'N/A' }}</div>
                                </div>
                                ...
                                <div
                                    class="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-100 dark:border-gray-700 shadow-sm">
                                    <div class="flex items-center mb-2">
                                        <x-icon name="o-map-pin" class="h-4 w-4 text-blue-500 mr-2" />
                                        <div class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                                            Coordinates</div>
                                    </div>
                                    <div class="flex flex-col text-gray-900 dark:text-white font-medium">
                                        <div class="flex items-center mb-1">
                                            <span
                                                class="text-xs text-gray-500 dark:text-gray-400 w-16">Latitude:</span>
                                            <span>{{ $viewCountry->latitude ?: 'N/A' }}</span>
                                        </div>
                                        <div class="flex items-center">
                                            <span
                                                class="text-xs text-gray-500 dark:text-gray-400 w-16">Longitude:</span>
                                            <span>{{ $viewCountry->longitude ?: 'N/A' }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <!-- Codes Tab -->
                        <div x-show="activeTab === 'codes'" class="py-3 space-y-3">
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                ...
                                <div
                                    ...
                                    <div class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">
                                        Numeric Code</div>
                                    <div class="text-gray-900 dark:text-white font-medium">
                                        {{ $viewCountry->numeric_code ?: 'N/A' }}</div>
                                </div>
                                ...
                                <div
                                    ...
                                    <div class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">
                                        Phone Code</div>
                                    <div class="text-gray-900 dark:text-white font-medium">
                                        @if ($viewCountry->phonecode)
                                            +{{ $viewCountry->phonecode }}
                                        @else
                                            N/A
                                        @endif
                                    </div>
                                ...
                                <div
                                    ...
                                    <div class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">
                                        TLD</div>
                                    <div class="text-gray-900 dark:text-white font-medium">
                                        {{ $viewCountry->tld ?: 'N/A' }}</div>
                                </div>
                            </div>
                        </div>

                        <!-- Currency Tab -->
                        <div x-show="activeTab === 'currency'" class="py-3 space-y-3">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <!-- Currency Code -->
                                <div
                                    class="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-100 dark:border-gray-700 shadow-sm">
                                    <div class="flex items-center mb-2">
                                        <x-icon name="o-currency-dollar" class="h-4 w-4 text-blue-500 mr-2" />
                                        <div class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                                            Currency Code</div>
                                    </div>
                                    <div class="text-gray-900 dark:text-white font-medium">
                                        {{ $viewCountry->currency ?: 'N/A' }}</div>
                                </div>
                                ...
                                <div
                                    class="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-100 dark:border-gray-700 shadow-sm">
                                    <div class="flex items-center mb-2">
                                        <x-icon name="o-banknotes" class="h-4 w-4 text-blue-500 mr-2" />
                                        <div class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                                            Currency Name</div>
                                    </div>
                                    <div class="text-gray-900 dark:text-white font-medium">
                                        {{ $viewCountry->currency_name ?: 'N/A' }}</div>
                                </div>

                                <!-- Currency Symbol -->
                                <div
                                    class="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-100 dark:border-gray-700 shadow-sm sm:col-span-2">
                                    <div class="flex items-center mb-2">
                                        <x-icon name="o-currency-dollar" class="h-4 w-4 text-blue-500 mr-2" />
                                        <div class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                                            Currency Symbol</div>
                                    ...
                                    <div class="text-gray-900 dark:text-white font-medium text-xl">
                                        {{ $viewCountry->currency_symbol ?: 'N/A' }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <!-- Timestamps Footer -->
                <div
                    class="bg-gray-50 dark:bg-gray-800 px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex flex-wrap gap-4 text-xs text-gray-500 dark:text-gray-400">
        ...
        @else
            ...
            <div class="bg-white dark:bg-gray-900 rounded-lg overflow-hidden animate-pulse">
                <!-- Hero Banner Skeleton -->
                <div class="bg-gray-300 dark:bg-gray-700 h-32 relative">
                    <div class="absolute right-4 top-4 h-16 w-16 bg-gray-400 dark:bg-gray-600 rounded-lg"></div>
                    <div class="absolute bottom-6 left-6">
                        <div class="h-8 bg-gray-400 dark:bg-gray-600 rounded w-48"></div>
                    </div>
                </div>

                <!-- Quick Stats Skeleton -->
                <div class="bg-gray-100 dark:bg-gray-800 p-4 flex flex-wrap justify-between">
                    @foreach (range(1, 3) as $_)
                        <div class="flex items-center">
                            <div class="h-8 w-8 bg-gray-300 dark:bg-gray-700 rounded-full mr-2"></div>
                            <div>
                                <div class="h-3 bg-gray-300 dark:bg-gray-700 rounded w-16 mb-1"></div>
                ...
                <div class="p-6">
                    ...
                    <div class="flex border-b border-gray-200 dark:border-gray-700 mb-6">
                        @foreach (range(1, 4) as $_)
                            <div class="h-8 bg-gray-300 dark:bg-gray-700 rounded w-20 mx-2"></div>
                        @endforeach
                    </div>
        ...
        <x-slot:actions>
            <div class="flex justify-end gap-3 w-full">
                @if ($viewCountry && !$viewCountry->deleted_at)
                    <x-button icon="o-pencil" label="Edit" wire:click="openModal({{ $viewCountry->id }})"
                        class="btn-primary" spinner />
                @endif
                <x-button icon="o-x-mark" label="Close" wire:click="closeModal" class="btn-outline" />
            </div>
        </x-slot:actions>
    </x-modal>
